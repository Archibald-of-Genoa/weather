...
import type { CityGeoData, ForecastData } from "../types";
import { getForecast } from "../client";
...
const SearchBox = () => {
    ...
    const handleCitySelect = (
        selectedCity: CityGeoData,
        isForecast: boolean = false
    ) => {
        setCity(selectedCity.name);
        setShowDropdown(false);
        setSelectedCityIndex(-1);
        getForecast({ city: selectedCity.name, isForecast }).then(data => {
            if (data) setForecastData(data);
        });
    };
