import type { WeatherDataProps, WeatherItem } from "../types";

const WeatherCard = ({ data }: WeatherDataProps) => {
    if (data.list) {
        const groupedByDate = data.list.reduce<Record<string, WeatherItem[]>>(
            (acc, elem) => {
        ...
        return (
            <div className="space-y-6">
                <h3 className="text-xl font-bold text-orange-800 mb-6 text-center">
                    {" "}
                    The weather forecast for {data.city?.name} in the next 5
                    days{" "}
                </h3>
                {Object.entries(groupedByDate)
                    ...
                    <div
                        key={date}
                        className="bg-gradient-to-r from-orange-200/60 to-amber-200/60 backdrop-blur-sm rounded-3xl p-6 shadow-lg"
                    >
                        <h3 className="text-xl font-bold text-orange-800 mb-6 text-center">
        ...
        <div className="bg-gradient-to-r from-orange-200/80 to-amber-200/80 backdrop-blur-sm rounded-3xl p-8 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-gradient-to-br from-amber-100 to-yellow-100 p-6 rounded-2xl shadow-md">
                    ...
                    <p className="text-xl font-medium text-amber-800 capitalize">
